...
- `timeout`、`upstream`：不做传输层自动重试，可由后续采集重新尝试；90 秒超时覆盖响应正文；
- `invalid_response`：空响应或截断允许一次备用请求；字幕引用不匹配允许一次引用修复。其他 JSON 或结构错误直接失败。
...
视频字幕摘要与观点通常在同一次 DeepSeek 请求中生成，包含备用请求和引用修复时最多四次，每次 90 秒。手动重试的 Nginx 等待时间为 420 秒；修改配置后必须验证并重载 Nginx。

API 手动重试和 Worker 共用原子分析领取；运行中内容不会被重复领取或替换原文。10 分钟后可回收中断的分析，过期任务不能覆盖新任务结果。摘要只覆盖真实字幕；正文、元数据和历史成功内容不会自动补齐摘要。

统计实际调用次数使用 `deepseek_request_started`，每条对应一次 HTTP 请求尝试（不等于成功或计费次数）。用量按 `deepseek_response_usage` 汇总，包含空响应、截断及修复请求；不要与 `content_analysis_completed/failed` 的汇总 token 重复相加。没有收到用量的超时或断连仍需以供应商账单核对，不能视为零消耗。日志只记录请求关联 ID、模型、计数和用量，不记录请求正文或凭据。
