...
import { database, databaseDirectory as dataDir, legacyNotesPath } from "./connection.js";
...
function applyCurrentSchema(conn: DatabaseSync) {
  conn.exec(`
...
const migrationRegistry = [
  { name: currentSchemaMigration, apply: applyCurrentSchema }
] as const;
...
export async function verifyDatabaseSchema() {
  await fsp.mkdir(dataDir, { recursive: true });
  const problems = schemaProblems(database(), currentSchemaMigration);
  if (problems.length) {
    throw new Error(`Database schema is not current (${problems.join(", ")}). Run npm run migrate first.`);
  }
  schemaReady = true;
}
